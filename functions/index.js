...
async function commitDocumentDeletes(documents) {
  for (let offset = 0; offset < documents.length; offset += 400) {
    const batch = db.batch();
    documents.slice(offset, offset + 400).forEach(document =>
...
function verifyAkitoRules(snapshot, expectedRules) {
  const expected = new Map(expectedRules.map(rule => [rule.ref.id, rule.data]));
  const actual = new Map(snapshot.docs.map(document => [document.id, document.data()]));
  const errors = [];

  if (actual.size !== expected.size) {
    errors.push(`件数が${actual.size}件です（期待値${expected.size}件）`);
  }
  for (const [id, expectedData] of expected) {
    const actualData = actual.get(id);
    if (!actualData) {
      errors.push(`${id}がありません`);
      continue;
    }
    for (const [field, expectedValue] of Object.entries(expectedData)) {
      if (["createdAt", "updatedAt"].includes(field)) continue;
      if (JSON.stringify(actualData[field]) !== JSON.stringify(expectedValue)) {
        errors.push(`${id}.${field}が一致しません`);
      }
    }
  }
  for (const id of actual.keys()) {
    if (!expected.has(id)) errors.push(`想定外のrule ${id}があります`);
  }
  ...
  return { errors, periodCounts, actual };
}
...
exports.syncAkitoRewardRules = onCall({ region: REGION }, async request => {
  ...
  const verifiedSnapshot = await db.collection("rules")
    .where("childId", "==", "akito")
    .get();
  const verification = verifyAkitoRules(verifiedSnapshot, rules);
  const ruleIds = [...verification.actual.keys()].sort();

  if (verification.errors.length) {
    logger.error("Akito reward rule verification failed", {
      errors: verification.errors,
      deletedCount: existing.size,
      createdCount: rules.length,
      verifiedCount: verifiedSnapshot.size,
      periodCounts: verification.periodCounts,
      ruleIds
    });
    throw new HttpsError(
      "internal",
      `反映後の確認に失敗しました：${verification.errors.join(" / ")}`
    );
  }
  ...
  logger.info("Akito reward rules synchronized", {
    ...
    createdCount: rules.length,
    verifiedCount: verifiedSnapshot.size,
    periods: verification.periodCounts,
    ruleIds
  });
  return {
    deletedCount: existing.size,
